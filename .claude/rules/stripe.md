...
- Every event is processed through `runOnce` against `public.webhook_events`, so Stripe replays and concurrent deliveries are skipped and failed events are retried on the next delivery. Handlers must still be safe to re-run (upserts), because the ledger marks an event `failed` if they throw.
