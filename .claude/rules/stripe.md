...
- The webhook (`src/app/api/webhooks/stripe/route.ts` → `features/billing/webhook-handlers.ts`) is the **only writer** of `products`, `prices`, `subscriptions`, and `customers` (the latter also via `features/billing/customers.ts` on first checkout). Never write these tables from pages or other actions.
