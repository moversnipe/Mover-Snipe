...
- Keep the admin client out of features except where the row cannot be read or written under RLS on purpose (`billing/customers.ts`, `billing/webhook-handlers.ts`, `scraping/jobs.ts`). Comment why each time.
