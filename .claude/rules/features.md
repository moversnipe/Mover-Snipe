...
| `queries.ts`       | Server-only reads (`import "server-only"`), wrapped in `React.cache`. Return typed rows; throw on Supabase errors.                                                             |
...
| other `*.ts`       | Capability modules split from `actions.ts` (`checkout.ts`, `password.ts`), pure helpers (`format.ts`, `redirect.ts`), or integrations (`customers.ts`, `webhook-handlers.ts`). |
...
- Keep the admin client out of features except where the row cannot be read or written under RLS on purpose (`billing/customers.ts`, `billing/webhook-handlers.ts`, `scraping/webhook-handlers.ts`, `scraping/completion.ts`). Comment why each time.
