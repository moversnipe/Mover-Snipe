...
| Module                                   | Purpose                                                                                                                                                                                                        |
| ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `lib/env/client.ts`, `lib/env/server.ts` | Zod-validated environment. The only place `process.env` is read. `server.ts` imports `server-only`. Supabase keys are validated as `sb_publishable_`/`sb_secret_` (legacy JWTs tolerated for the local stack). |
