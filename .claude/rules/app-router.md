...
- `(app)/layout.tsx` is the sidebar shell: `SidebarProvider` (its `defaultOpen` read from the `SIDEBAR_STATE_COOKIE` cookie), `AppSidebar` (Base UI `variant="inset"`, so the sidebar has no panel background and `SidebarInset` becomes a rounded floating card), and a borderless `SidebarInset` header holding the trigger and breadcrumb. Theme and sign-out belong to the account card in the sidebar footer (`features/auth/components/nav-user.tsx`), not the header. The layout reads `getProfile` for that card, which needs the id from `requireUser()` and so cannot join the `Promise.all`. `AppBreadcrumb` renders nothing (its leading divider included) for a path outside `NAV_SECTIONS`. `SidebarInset` already renders `<main>`, so a page must not add its own. A new page under `(app)` gets a `NavItem` in `NAV_SECTIONS` (`src/config/navigation.ts`) to appear in the sidebar.
