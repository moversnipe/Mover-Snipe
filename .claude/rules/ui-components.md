...
- The `base-nova` style ships a denser sidebar than earlier shadcn styles: `SidebarContent` and `SidebarMenu` both carry `gap-0`, which puts groups and entries flush against each other. `src/components/app-sidebar.tsx` restores the spacing at the call site with `gap-2` on the content column and `gap-1` on each menu. Re-running the shadcn CLI resets the vendored defaults but leaves these overrides intact; keep them rather than editing `src/components/ui/sidebar.tsx`.
