...
# The local JWT signing key (supabase/signing_keys.json) and any *.pem / *.key
# file may be generated by the documented npm script, but never read, copied,
# or printed by an agent.
if matches 'signing_keys[A-Za-z0-9_.-]*\.json'; then
  if ! matches '^npm[[:space:]]+run[[:space:]]+db:signing-key([[:space:]]|$)'; then
    deny "that command touches a private JWT signing key file. Only 'npm run db:signing-key' may create it; never read or copy it."
  fi
fi
if matches "${cmd_start}(cat|less|more|head|tail|bat|sed|awk|grep|rg|cut|xxd|od|strings|base64|cp|scp|rsync|curl|python3?|node)[[:space:]]${same_cmd}[A-Za-z0-9_./-]+\.(pem|key)([^A-Za-z0-9_.-]|$)"; then
  deny "that command reads certificate or private key material."
fi
...
if matches 'supabase[[:space:]]+(functions[[:space:]]+deploy|secrets[[:space:]]+set)'; then
  deny "deploying Edge Functions or setting hosted secrets is the user's manual step."
fi
