...
if matches 'supabase[[:space:]]+db[[:space:]]+reset.*--(linked|db-url)'; then
...
if matches 'git[[:space:]]+push.*([[:space:]]-f([[:space:]]|$)|--force)'; then
  deny "force-pushing rewrites shared history. Push normally or ask the user."
fi
if matches 'rm[[:space:]]+-[a-zA-Z]*r[a-zA-Z]*[[:space:]].*supabase/migrations'; then
  deny "deleting migrations breaks every environment that already applied them."
fi

# Reading or copying secret env files (.env.example is fine).
...
if matches "(^|[;&|(][[:space:]]*)(sudo[[:space:]]+)?${readers}[[:space:]][^;&|]*${secret_env}"; then
