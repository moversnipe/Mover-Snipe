...
2. Write the SQL: header comment, `create table public.<plural>`, `comment on table`, `enable row level security`, one policy per operation with `to` roles and `(select auth.uid())`, indexes on policy and FK columns, `updated_at` trigger via `public.set_updated_at()`.
