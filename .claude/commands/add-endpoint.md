...
1. Decide the kind. JSON API → `src/app/api/<resource>/route.ts`. Webhook → `src/app/api/webhooks/<provider>/route.ts` plus `src/lib/<provider>/webhooks.ts` (signature verification) and `src/features/<domain>/webhook-handlers.ts` (event handling and all writes). Redirect callback → `src/app/auth/<flow>/route.ts`.
